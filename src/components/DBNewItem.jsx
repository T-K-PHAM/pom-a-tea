...
import { getAllBbtItems, saveItem } from "../utils/firebaseFunctions";
...
const DBNewItem = () => {
  ...
  const saveDetails = () => {
    setisLoading(true);
    try {
      if (!itemName || !calories || !imageDownloadURL || !price || !category) {
        setFields(true);
            ...
            <div className="w-full flex items-center justify-around gap-3 flex-wrap">
                {statuses && statuses?.map(data => (
                    ...
                    font-semibold cursor-pointer hover:shadow-md border
                     ...
                     ${data.category === category ? "bg-pink-400 text-primary" : "bg-transparent"}`}
                     >
                        {data.title}
                     </p>
                ))}
